...
export class TaskLocalFileTransform extends QueueTask {
  ...
  constructor(
    protected override logger: winston.Logger,
  ...
  async generateBinary(binaryFile: BinaryFile) {
    await new Promise<void>((resolve, reject) => {
      try {
        const dest = this.destinationDirectory.createWriteStream(
          this.realFileName,
        );
        ...
        const ext = googleMimeToExt(
          this.googleFile.mimeType,
          this.googleFile.name,
        );
        const stream = this.googleFolder.createReadStream(
          `${binaryFile.id}${ext ? '.' + ext : ''}`,
        )
          .on('error', (err) => {
            reject(err);
          })
  ...
  async generateDrawing(drawingFile: DrawingFile) {
    await new Promise<void>((resolve, reject) => {
      try {
        ...
        stream.on('finish', () => {
          this.localLinks.append(
            drawingFile.id,
            drawingFile.fileName,
            Array.from(svgTransform.links),
          );
          resolve();
        });
        stream.on('error', (err) => {
  ...
  async generateDocument(localFile: MdFile) {
    ...
    const odtPath = this.googleFolder.getRealPath() + '/' + localFile.id +
      '.odt';
    const destinationPath = this.destinationDirectory.getRealPath();

    const rewriteRules = this.userConfig.rewrite_rules || [];

    const picturesDirAbsolute = destinationPath + '/' +
      this.realFileName.replace(/.md$/, '.assets/');

    let workerResult: WorkerResult;

    if ('kerebron' === this.markdownMode) {
      workerResult = await this
        .jobManagerContainer.scheduleWorker('OdtToMarkdownKerebron', {
          localFile,
      ...
      workerResult = await this
        .jobManagerContainer.scheduleWorker('OdtToMarkdown', {
          localFile,
          realFileName: this.realFileName,
          picturesDirAbsolute,
          odtPath,
    ...
    for (const errorMsg of errors) {
      this.logger.warn(
        'Error in: [' + this.localFile.fileName + '](' +
          this.localFile.fileName + ') ' + errorMsg,
        {
    ...
    await this.destinationDirectory.writeFile(
      this.realFileName,
      frontMatter + markdown,
    );
    this.localLinks.append(localFile.id, localFile.fileName, links);
    for (const k in headersMap) {
      this.globalHeadersMap['gdoc:' + localFile.id + k] = 'gdoc:' +
        localFile.id + headersMap[k];
    }
    for (const k in invisibleBookmarks) {
      this.globalInvisibleBookmarks['gdoc:' + localFile.id + k] =
        invisibleBookmarks[k];
    }
  }

  async generate(localFile: LocalFile): Promise<void> {
    try {
      const verStr = this.localFile.version
        ? ' #' + this.localFile.version
        : ' ';
      if (localFile.type === 'conflict') {
        ...
        this.logger.info(
          'Transforming markdown: ' + this.localFile.fileName + verStr,
        );
        ...
        this.logger.info(
          'Transforming drawing: ' + this.localFile.fileName + verStr,
        );
        // const googleFile = await this.googleScanner.getFileById(localFile.id);
        // const downloadFile = await this.downloadFilesStorage.findFile(f => f.id === localFile.id);
        if (this.googleFile) { // && downloadFile
        ...
        this.logger.info(
          'Transforming binary: ' + this.localFile.fileName + verStr,
        );
        if (this.googleFile) { // && downloadFile
      ...
      this.logger.error(
        'Error transforming ' + localFile.fileName + ' ' + err.stack
          ? err.stack
          : err.message,
      );
      throw err;
